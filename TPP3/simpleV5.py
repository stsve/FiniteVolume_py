...
#%%
def bcValue(bc):
    global bcdata, bcdata_u,bcdata_v,bcdataP,xmin,xmax,ymin,ymax,fu,fv,fVel,fP,funPress,P
    mu = 0.1
    dp = 1200
...
#    fP = P
    ...
    bcdata = (['Libre',0],['Neumann',0],['Dirichlet',fP],['Neumann',0])
...
def varInit(divX,divY):
    global xx,yy,tri,are,cenTri,MTri1,fig1,ax1,k,n_node,centri
    ...
    for a in are:
        ...
        if a[3]==-1 and bcdata[a[4]][0] == 'Dirichlet':
            if callable(bcdata[a[4]][1]):
                ...
                b[a[2]][0,0] += (mAx[u]-centri[a[2],0])*(bcdata[a[4]][1]-phiIni[a[2]])
                b[a[2]][1,0] += (mAy[u]-centri[a[2],1])*(bcdata[a[4]][1]-phiIni[a[2]])
...
#    Sy = fSy(X,Y)
#
    qx = Sx.ravel()*centri[:,0]
    qy = Sy.ravel()*centri[:,1]
