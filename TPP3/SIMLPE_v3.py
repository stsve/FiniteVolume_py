...
#    v = -(2*y**2 - y**4 - 1)*(x-x**3)
    ...
    funPress = sp.simplify((0*x**2+0*y+dp))
...
def msolv2D(numelx,numely):
    global PNKSI, PNN,A,ETA,KSI,deta,vel,X,Y,solU,solV,tim2,flag,solPhip,fSx,fSy,F,AA,aireTri,uF,P,gradP,bx,Sxx,MP,BP,Pprime,qx,qy,Sx,Sy
    ...
    it_sim = 0
    while it_sim < nb_iter:
        
        MP = np.zeros([np.size(tri,0),np.size(tri,0)])
        BP = np.zeros((np.size(tri,0)))
        Pprime = np.zeros(np.size(tri,0))
        uFp = np.zeros((np.size(are,0)))
        uFpp = np.zeros((np.size(are,0)))
        
        pbar.set_description('Processing iteration '+ str(it_sim ))
...
#        if it_sim > 1:
#            uFp = alphaRC*uFp + (1-alphaRC)*uFpp
        ...
        for a in are:
            if a[3]!=-1:
                ...
                uF[u] += dfi*(Pprime[a[2]]-Pprime[a[3]])
