...
#    u = (2*x**2 - x**4 - 1)*(y-y**3)
    u = 1 + 0*x+0*y
    v = -0*(2*y**2 - y**4 - 1)*(x-x**3)
    velTot = sp.simplify((u**2 + v**2)**(1/2))
    funPress = sp.simplify((x**2+y))
    fVel = sp.lambdify([x,y],velTot,'numpy')
    fP = sp.lambdify([x,y],funPress,'numpy')
    
    fu = np.vectorize(sp.lambdify([x,y],u,'numpy'))
    fv = np.vectorize(sp.lambdify([x,y],v,'numpy'))
...
#    xx,yy,tri,are = mec6616.RectGmsh(xmin,xmax,ymin,ymax,(xmax-xmin)/divx)
    ...
    bcdata_u = (['Dirichlet',fu],['Dirichlet',0],['Dirichlet',fu],['Dirichlet',0])
...
def msolv2D(numelx,numely):
    global PNKSI, PNN,A,ETA,KSI,deta,vel,X,Y,solU,solV,tim2,flag,solPhip,fSx,fSy,F,AA,aireTri,uF,Pcore,MP,BP,DKSI,Sx,Sy
...
#    P = np.sqrt(np.square(qx)+np.square(qy))
    ...
    qx = np.zeros((np.size(tri,0)))
    qy = np.zeros((np.size(tri,0)))
    
    uF = np.zeros((np.size(are,0)))
    uFp = np.zeros((np.size(are,0)))
    
    MP = np.zeros([np.size(tri,0),np.size(tri,0)])
    Pcore = np.zeros(np.size(tri,0))
    ...
    while itt < nbit:
        ...
        for i in are:
            ...
            if i[3] != -1:
                ...
                if callable(bcdata_u[i[4]][1]) and callable(bcdata_v[i[4]][1]):
                    F[u] = np.dot(N[u,:],np.array([(rho*bcdata_u[i[4]][1](mAx[u],mAy[u])),(rho*bcdata_v[i[4]][1](mAx[u],mAy[u]))]))*deta[u]
                else:
                    F[u] = np.dot(N[u,:],np.array([(rho*bcdata_u[i[4]][1]),(rho*bcdata_v[i[4]][1])]))*deta[u]
...
#        uF = np.zeros((np.size(are,0)))
    ...
    #            uF[u] = np.dot((1/2)*(nU[a[2]]+nU[a[3]]),N[u]) + diffP + np.dot(diffgradP,KSI[u,:])
        ...
        for a in are:
            ...
            if (a[3]==-1 and bcdatap[a[4]][0]=='Sortie'): #and bcdata_v[a[4]][0] =='Dirichlet'):
                dVP = aireTri[a[2]]*1
                aP = A[a[2],a[2]]
                dfi = dVP/(aP*DKSI[u])
                MP[a[2],a[2]] += rho*dfi*dA[u]
            ...
            if (a[3]==-1 and (bcdatap[a[4]][0]=='Entree' or bcdatap[a[4]][0]=='Paroi')):
                ytt=0
                
            u+=1
        
        MP_sp = sprs.csr_matrix(MP)
        Pcore = spsolve(MP_sp,BP)
        ...
        for a in are:
            if a[3]!=-1:
                ...
                uF[u] += dfi*(Pcore[a[2]]-Pcore[a[3]])
                
            if (a[3]==-1 and bcdata[a[4]][0]=='Sortie'): #and bcdata_v[a[4]][0] =='Dirichlet'):
                dVP = aireTri[a[2]]*1
                aP = A[a[2],a[2]]
                dfi = dVP/(aP*DKSI[u])
                uF[u] += dfi*(Pcore[a[2]])
                
            if (a[3]==-1 and (bcdata[a[4]][0]=='Entree' or bcdata[a[4]][0]=='Paroi')):
#                uF[u] = np.dot(UU[a[2]],N[u]) + (dVP/aP)*(P[a[2]]-bcdata[a[4]][1](mAx[u],mAy[u]))/DKSI[u] + (dVP/aP)*np.dot(gradPhi[a[2]],KSI[u,:])
                ytt=0
        ...
        P += alphaP*Pcore
        gradPhi = gradLS(P)
        gradPhi_mod = gradLS(P).transpose(2,0,1).reshape(-1,2)
...
fgradx = sp.lambdify([x,y],Gradx,'numpy')
fgrady = sp.lambdify([x,y],Grady,'numpy')
...
S = np.sqrt(np.square(Sxx)+np.square(Syy))
