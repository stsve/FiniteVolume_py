...
def msolv2D(numelx,numely):
    global PNKSI, PNN,A,ETA,KSI,deta,vel,X,Y,solU,solV,tim2,flag,solPhip,fSx,fSy,F,AA,aireTri,uF,Pcore,MP,BP,DKSI
...
#            solV = np.linalg.solve(A,B2)
    ...
    while itP < 5:
        ...
        for a in are:
            if a[3]!=-1:
                ...
                diffPcore = (1/2)*(dVP/aP + dVA/aA)*(Pcore[a[2]]-Pcore[a[3]])*(1/DKSI[u])
    ...
    #            uF[u] = np.dot((1/2)*(nU[a[2]]+nU[a[3]]),N[u]) + diffP + np.dot(diffgradP,KSI[u,:])
                uF[u] = np.dot([(1/2)*(UU[a[2],:]+UU[a[3],:])],N[u]) + diffP + np.dot(diffgradP,KSI[u,:]) + diffPcore
                
                MP[a[2],a[2]] += rho*diffP*dA[u]
                MP[a[2],a[3]] -= rho*diffP*dA[u]
                MP[a[3],a[3]] += rho*diffP*dA[u]
                MP[a[3],a[2]] -= rho*diffP*dA[u]
            ...
            if (a[3]==-1 and bcdata_u[a[4]][0]=='Dirichlet' and bcdata_v[a[4]][0] =='Dirichlet'):
                ...
                aP = A[a[2],a[2]]
                dfi = dVP/(aP*DKSI[u])
                uF[u] = np.dot(np.array([bcdata_u[a[4]][1](mAx[u],mAy[u]),bcdata_v[a[4]][1](mAx[u],mAy[u])]),N[u,:]) + dfi*(Pcore[a[2]])
