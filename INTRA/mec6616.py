...
"""
FONCTION :  QuarterAnnularGmsh(R1,R2,lc)

Maillage d'un quart d'anneau en triangles avec gmsh
Eddy Petro et Jean-Yves Trépanier - Janvier 2020

Utilise la fonction TriAretes du module mec6616. 
    ...
    R1     ------               \
       ...
       cf=1         \            |
                     ...
                     |__________ |
                         ...
                         cf=2                             
INPUT:
R1   : Rayon du cercle interieur
R2   : Rayon du cercle extérieur
lc   : Longueur visée des arètes des triangles
OUTPUT: 
xx, yy : array de coordonnées des noeuds du maillage
triangles : connectivité des triangles : n0,n1,n2 trigo pour chaque triangle
...
"""
def QuarterAnnularGmsh(R1,R2,lc):
    ...
    # Next we add a new model named "t1" (if gmsh.model.add() is not called a new
    # unnamed model will be created on the fly, if necessary):
    gmsh.model.add("QuarterAnnular")
      
    gmsh.model.geo.addPoint(R1,0,0,lc,1)
    gmsh.model.geo.addPoint(0,R1,0,lc,2)
    gmsh.model.geo.addPoint(R2,0,0,lc,3)
    gmsh.model.geo.addPoint(0,R2,0,lc,4)
    ...
    gmsh.model.geo.addCircleArc(2,5,1,4)
    ...
    for iare in range(0,aretef.shape[0]):
        ...
        if(xmid < 0.1*lc):
            ...
            aretef[iare,4]=2
        elif((xmid**2+ymid**2)**0.5 > R2-0.1*lc):
            aretef[iare,4]=3
        elif((xmid**2+ymid**2)**0.5 < R1+0.1*lc):
            aretef[iare,4]=1
