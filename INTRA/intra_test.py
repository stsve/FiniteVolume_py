...
# %%     Définition des conditions du problème

#--------------------------------------------------------------------#
#            Champs de vitesse et température pour MMS               #
#--------------------------------------------------------------------#
...
#s= sp.symbols('s')
#T = sp.tanh(-20.*(y-1.6))-sp.tanh( -20*(y-1.4))
#sp.plot(T,(y,1,2))
...
uu = np.vectorize(sp.lambdify([x,y], U,'numpy'))
vv = np.vectorize(sp.lambdify([x,y], V,'numpy'))

#--------------------------------------------------------------------#
#                      Derivation pour MMS                           #
#--------------------------------------------------------------------#

conv = sp.simplify(sp.diff(U*T,x)+sp.diff(V*T,y))
diff = sp.simplify(sp.diff(sp.diff(T,x),x)+sp.diff(sp.diff(T,y),y))
source = 0*rho*Cp*conv - 0*gamma*diff


fSrc = np.vectorize(sp.lambdify([x,y],source,'numpy'))

#--------------------------------------------------------------------#
#                Type de maillage? (Gmsh ou Rect)                    #
#--------------------------------------------------------------------#
...
def varInit(divX,divY):
    ...
    xmax = 0.1;                           # Limite supérieure du domaine en x
    ymin = 0;                          # Limite inférieure du domaine en y
    ymax = 0.05;                           # Limite supérieure du domaine en y
...
#    
    ...
    if gmsh:
#        xx,yy,tri,are = mec6616.RectGmsh(xmin,xmax,ymin,ymax,(xmax-xmin)/(1.5*divx))
        xx,yy,tri,are = mec6616.QuarterAnnularGmsh(1,2,(2-1)/divisions)
    else:
...
#       annoter les noeuds
    ...
    bcdata = (['Dirichlet',fT],['Dirichlet',0],['Neumann',0],['Neumann',0])
...
def solveur2D(numelx,numely):
    global PNKSI, PNN,A,ETA,KSI,deta,vel,X,Y,solPhi,tim2,flag,solPhip,Source,q,aireTri
...
#   q = Source
    ...
    q = fSrc(X,Y)       # q -> Source tirée de MMS
    ...
    for i in are:
        if (i[3] == -1 and bcdata[i[4]][0]=='Dirichlet'):
            if callable(bcdata[i[4]][1]):
                b[i[2]] += D[u]*bcdata[i[4]][1](mAx[u],mAy[u]) + max(0,-F[u])*bcdata[i[4]][1](mAx[u],mAy[u])
            else:
                b[i[2]] += D[u]*bcdata[i[4]][1] + max(0,-F[u])*bcdata[i[4]][1]
...
triangulation = False
upwind =  True
A,b = solveur2D(divisions,divisions)
...
sortedY = np.sort(Y)
borne_sup = sortedY[divisions]

tricoupe = np.where(Y<borne_sup)
x_coupe = np.zeros(divisions)
phi_coupe = np.zeros(divisions)

u=0
for i in tricoupe[0]:
    x_coupe[u] = X[i]
    phi_coupe[u] = solPhi[i]
    
    u+=1

fig3, ax3 = plt.subplots()
ax3.plot(x_coupe,phi_coupe,'bo')
ax3.set_title('Coupe en x en sortie du coude \n Upwind: '+str(upwind))
ax3.set_xlabel('X axis')
ax3.set_ylabel('Température (T)')
plt.show()
...
#%%
#--------------------------------------------------------------------#
#              Ordre de convergence - Méthode de Célik               #
#--------------------------------------------------------------------#

nraff = np.array([10,20,40]) # Nombre de nelm doit etre egal au size de "solutions"

solutions = [[],[],[]]      # Minimum 3 maillages
H = [[],[],[]]

tri_entree = [[],[],[]]
tri_sortie = [[],[],[]]

MoyPhi = np.zeros([np.size(nraff)])

Norme = np.zeros((np.size(nraff)))
Ordre = np.zeros((np.size(nraff))-1)

for i in range(0,np.size(nraff)):
    nelm = nraff[i]
    solveur2D(nelm,nelm)
    solutions[i] = solPhi
    H[i] = np.sqrt(1/np.size(tri,0)*np.sum(aireTri))
    MoyPhi[i] = np.average(solPhi)
    
    sortedY = np.sort(Y)
    sortedX = np.sort(X)
    borne_supy = sortedY[nelm]
    borne_supx = sortedX[nelm]

    tricoupe_s = np.where(Y<borne_supy)
    tricoupe_e = np.where(X<borne_supx)
    x_coupe = np.zeros(nelm)
    y_coupe = np.zeros(nelm)
    phi_coupex = np.zeros(nelm)
    phi_coupey = np.zeros(nelm)

    u=0
    for ii in tricoupe_s[0]:
        x_coupe[u] = X[ii]
        phi_coupex[u] = solPhi[ii]
    
        u+=1
    
    tri_sortie[i] = phi_coupex
        
    u=0 
    for ii in tricoupe_e[0]:
        y_coupe[u] = X[ii]
        phi_coupey[u] = solPhi[ii]
        u+=1
        
    tri_entree[i] = phi_coupey
    
    Norme[i] = np.sqrt((1/np.size(tri,0))*np.sum(np.square(tri_sortie[i]-tri_entree[i])))
...
#p1 = np.log((np.average(solutions[4])-np.average(solutions[2]))/(np.average(solutions[2])-np.average(solutions[0])))/np.log(ratio1)
#p2 = np.log((np.average(solutions[3])-np.average(solutions[2]))/(np.average(solutions[2])-np.average(solutions[1])))/np.log(ratio3)
#p3 = np.log((np.average(solutions[4])-np.average(solutions[3]))/(np.average(solutions[3])-np.average(solutions[2])))/np.log(ratio4)

p = np.log((Norme[2]-Norme[1])/(Norme[1]-Norme[0]))/np.log(ratio1)

#fig3,ax3 = plt.subplots()
#ax3.plot(nraff,MoyPhi,'-ok')
#plt.show()

print('L''ordre de convervence est de p=',p)
